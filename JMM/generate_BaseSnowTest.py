...
path_info = '/Users/marianamartinez/Documents/NYU-Courant/FMM-Project/TestBaseSnow/'
...
tan_points = [(0,0)] # the starting point is NOT on the boundary
points.extend([ (points_base[i, 0], points_base[i, 1]) for i in range( len(points_base) ) ] )
tan_points.extend([ (tan_base[i, 0], tan_base[i, 1]) for i in range( len(points_base) ) ] )
# Add the facets
facets = round_trip_connect(1, len(points) -1 ) # we don't want to connnect x0
boundary_chain = [ (-1,-1), (len(points), 2)] + [ (i-1, i+1) for i in range(2, len(points) -1) ] + [(len(points)-1, 1)]
...
# Add 0 to those points on the mesh that are not on the boundary and -1 to boundary chain
tan_points.extend( [0,0] for i in range( N_points_square - len(points_base) -1 ) )
boundary_chain.extend( [-1,-1] for i in range( N_points_square - len(points_base) -1 ) )
...
np.savetxt(path_info + currentH + '/'+ currentH +'_BoundaryPoints.txt', np.array(edges_square), delimiter =', ', fmt = '%.8f' )

facets_arr = np.array(facets_square)
np.savetxt(path_info + currentH + '/'+ currentH +'_Facets.txt', facets_arr.astype(int), delimiter =', ', fmt ='%.0f' )
...
np.savetxt(path_info + currentH + '/'+ currentH +'_Boundary_tan.txt', tan_points, delimiter =', ', fmt = '%.8f' )

boundary_cArray = np.array(boundary_chain)
np.savetxt(path_info + currentH + '/'+ currentH +'_Boundary_chain.txt', boundary_cArray.astype(int), delimiter =', ', fmt = '%.0f' )
...
with open(path_info + currentH + '/'+ currentH + "_FacesLabel.txt", "w") as out_file:
