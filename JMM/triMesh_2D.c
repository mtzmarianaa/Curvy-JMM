...
void triMesh2_init(triMesh_2Ds *triM_2D, coordS *points, neighborsRS *neighbors, neighborsRS *incidentFaces, facesS *faces, int nPoints, int *indexRegions, double (*boundary_tan)[2], int (*boundary_chain)[2]){
    ...
    triM_2D->faces = faces;
    triM_2D->indexRegions = indexRegions;
    triM_2D-> boundary_tan = boundary_tan;
    triM_2D-> boundary_chain = boundary_chain;
} 
...
void triMesh2_init_from_meshpy(triMesh_2Ds *triM_2D, char const *pathPoints, char const *pathNeighbors, char const *pathIncidentFaces, char const *pathFaces, char const *pathIndexRegions, char const *pathBoundaryTan, char const *pathBoundaryChain){
    ...
    double (*boundary_tan)[2];
    boundary_tan = malloc(2*nPoints*sizeof(double));
    read_n2File_double(boundary_tan[0], pathBoundaryTan);
    printf("Read the boundary data");
    triM_2D->boundary_tan = boundary_tan;

    int (*boundary_chain)[2];
    boundary_chain = malloc(2*nPoints*sizeof(int));
    read_n2File_int(boundary_chain[0], pathBoundaryChain);
    triM_2D->boundary_chain = boundary_chain;
...
void printEverythingInMesh(triMesh_2Ds *triM_2D) {
    ...
    for(int i = 0; i < triM_2D->nPoints; i++){
      printf("Point number %d, tangent to boundary:  %lf | %lf\n", i, triM_2D->boundary_tan[i][0], triM_2D->boundary_tan[i][1] );
    }
    printf("\n\n---------------------------------------\n");
    printf("BOUNDARY CHAIN\n");
    for(int i = 0; i < triM_2D->nPoints; i++){
      printf("Point number %d, before:  %d | after: %d\n", i, triM_2D->boundary_chain[i][0], triM_2D->boundary_chain[i][1] );
    }
