...
void initializePointsNear(eik_gridS *eik_g, double rBall) {
  ...
  for(int j = 0; j<eik_g->nStart; j++){
    ...
    xStart[0] = eik_g->triM_2D->points[ indexStart ][0];
    xStart[1] = eik_g->triM_2D->points[ indexStart][1];
    ...
    for(int i = 0; i<eik_g->triM_2D->nPoints; i++){
      xCurrent[0] = eik_g->triM_2D->points[i][0];
      xCurrent[1] = eik_g->triM_2D->points[i][1];
