...
# The paths
path_read = "/Users/marianamartinez/Documents/NYU-Courant/FMM-Project/FMM/TestBaseSnow/"
...
if path_trueSol is not None:
    path_solution = path_trueSol + 'true_solGrid_' + str(nx) + '_' + str(ny) + '.txt'
    path_type = path_trueSol + 'type_Sol_' + str(nx) + '_' + str(ny) + '.txt'
...
if path_test1.is_file() and path_test2.is_file():
    ...
    # Save them
    np.savetxt('TestBaseSnow/true_solGrid_' + str(nx) + '_' + str(ny) + '.txt', true_solGrid, delimiter = ', ', fmt = '%.8f')
    np.savetxt('TestBaseSnow/type_Sol_' + str(nx) + '_' + str(ny) + '.txt', type_Sol.astype(int), delimiter = ', ', fmt = '%.0f')
...
for stringPart in Hs:
    ...
    ######
    ######      READ EVERYTHING
    times = np.fromfile(path_read + stringPart + "/" + stringPart + "_TimesCubic.bin")
    eik_vals = np.fromfile(path_read + stringPart + "/" + stringPart + "_ComputedValuesCubic.bin")
    ...
    eik_grads = np.fromfile(path_read + stringPart + "/" + stringPart + "_ComputedGradientsCubic.bin")
    eik_grads = eik_grads.reshape(len(eik_coords), 2)
    eik_parents = np.fromfile(path_read + stringPart + "/" + stringPart + "_ParentsCubic.bin", dtype=np.int32)
    eik_parents = eik_parents.reshape(len(eik_coords), 2)
    eik_lambdas = np.fromfile(path_read + stringPart + "/" + stringPart + "_LambdasOptCubic.bin")
