...
plt.savefig('/Users/marianamartinez/Documents/NYU-Courant/FMM-bib/Figures/TestBaseSnow/H8_5/H8_5_TriangulationWhite.png', dpi=my_dpi * 10)
...
plt.show(block = False)
plt.savefig('/Users/marianamartinez/Documents/NYU-Courant/FMM-bib/Figures/TestBaseSnow/H8_5/H8_5_Triangulation.png', dpi=my_dpi * 10)


## Save them as well

np.savetxt('TestBaseSnow/H8_5/H8_5_BoundaryPoints.txt', np.array(edges_square), delimiter =', ', fmt = '%.8f' )

facets_arr = np.array(facets_square)
np.savetxt('TestBaseSnow/H8_5/H8_5_Facets.txt', facets_arr.astype(int), delimiter =', ', fmt ='%.0f' )

np.savetxt('TestBaseSnow/H8_5/H8_5_MeshPoints.txt', mesh_square_points, delimiter =', ', fmt = '%.8f' )
print("Amount of new points: ", len(mesh_square_points))
np.savetxt('TestBaseSnow/H8_5/H8_5_Faces.txt', mesh_square_tris.astype(int), delimiter =', ', fmt ='%.0f' )

np.savetxt('TestBaseSnow/H8_5/H8_5_NeighTriangles.txt', mesh_square_neigTriangles.astype(int), delimiter =', ', fmt ='%.0f')

separator = "," 

with open("TestBaseSnow/H8_5/H8_5_Neigh.txt", "w") as out_file:
    for l in mesh_square_neigh:
        out_string = separator.join(str(x) for x in l) + "\n"
        out_file.write(out_string)
        
with open("TestBaseSnow/H8_5/H8_5_IncidentFaces.txt", "w") as out_file:
    for l in mesh_IncidentFaces_sq:
        out_string = separator.join(str(x) for x in l) + "\n"
        out_file.write(out_string)
        
with open("TestBaseSnow/H8_5/H8_5_FacesLabel.txt", "w") as out_file:
...
# ax.add_patch(circle_b)
# plt.title('Delaunay triangulation of test geometry (just base) with rectangle, H = '+h_string)
# plt.savefig('/Users/marianamartinez/Documents/NYU-Courant/FMM-bib/Figures/TestBaseSnow/H4/H4_TriangulationWhite.png', dpi=my_dpi * 10)
# plt.show(block=False)
...
# plt.title('Delaunay triangulation of test geometry (just base), H = '+h_string)
# plt.show(block = False)
# plt.savefig('/Users/marianamartinez/Documents/NYU-Courant/FMM-bib/Figures/TestBaseSnow/H4/H4_Triangulation.png', dpi=my_dpi * 10)


## Save them as well

# np.savetxt('TestBaseSnow/H4/H4_BoundaryPoints.txt', np.array(edges_square), delimiter =', ', fmt = '%.8f' )

# facets_arr = np.array(facets_square)
# np.savetxt('TestBaseSnow/H4/H4_Facets.txt', facets_arr.astype(int), delimiter =', ', fmt ='%.0f' )

# np.savetxt('TestBaseSnow/H4/H4_MeshPoints.txt', mesh_square_points, delimiter =', ', fmt = '%.8f' )
# print("Amount of new points: ", len(mesh_square_points))
# np.savetxt('TestBaseSnow/H4/H4_Faces.txt', mesh_square_tris.astype(int), delimiter =', ', fmt ='%.0f' )

# np.savetxt('TestBaseSnow/H4/H4_NeighTriangles.txt', mesh_square_neigTriangles.astype(int), delimiter =', ', fmt ='%.0f')

# separator = "," 

# with open("TestBaseSnow/H4/H4_Neigh.txt", "w") as out_file:
#     for l in mesh_square_neigh:
#         out_string = separator.join(str(x) for x in l) + "\n"
#         out_file.write(out_string)
        
# with open("TestBaseSnow/H4/H4_IncidentFaces.txt", "w") as out_file:
#     for l in mesh_IncidentFaces_sq:
#         out_string = separator.join(str(x) for x in l) + "\n"
#         out_file.write(out_string)
        
# with open("TestBaseSnow/H4/H4_FacesLabel.txt", "w") as out_file:
#     for l in faces_label:
#         out_string = separator.join(str(l)) + "\n"
#         out_file.write(out_string)
